...
\begin{enumerate}
	...
	\item From each \coqobj[s] of the form $\name[s] \objdef \term[s] : \type[s]$,
		...
		where $\termset[s]$\glsadd{termset} is the set of names of all objects in the term,
		and $\typeset[s]$\glsadd{typeset} is the set of all names in the type of the proof (what is proven).
