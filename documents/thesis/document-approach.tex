...
\begin{figure}[H]
	...
	\begin{tikzpicture}[auto, node distance=2.5cm, main/.style={draw,align=center}]
		...
		\node[main] (summary) [below of=term] {Summary\\$<p, B, D>$};
		...
		\draw[->] (summary) edge node [right] {$D$} (definitions);
		\draw[->] (summary) edge node [left] {$\text{dom}(S) / D$} (theorems);
		...
		\draw[dashed] (summary) edge [bend left, pos=0.19] node {$B$} (dependencies);
	\end{tikzpicture}
