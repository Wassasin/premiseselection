...
\begin{figure}[H]
	...
	\begin{tikzpicture}[auto, node distance=2.5cm, main/.style={draw,align=center}]
		...
		\node[main] (summary) [below of=term] {Summary\\$<p_s, t_s, A_s>$};
		...
		\draw[->] (summary) edge node [right] {$A_s$} (definitions);
		\draw[->] (summary) edge node [left] {$t_s / A_s$} (theorems);
		...
		\draw[dashed] (summary) edge [bend left, pos=0.19] node {$t_s$} (dependencies);
	\end{tikzpicture}
