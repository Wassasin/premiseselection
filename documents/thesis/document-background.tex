...
\begin{definition}[term]\glsadd{termclass}
	...
	The empty term is written as $\emptyterm$\glsadd{emptyterm}.
\end{definition}
...
\begin{definition}[sort]\defgls{sort}
	The type of a type when manipulated as a term is called a sort.
	\pcic uses an infinite well-founded typing hierarchy of sorts, with base sorts \sortprop\glsadd{sortprop} and \sortset\glsadd{sortset}, and with inductive sort \sorttype\glsadd{sorttype}.
...
A proof in \coq is a statement written in the \gallina\glsadd{gallina} language \cite{huet1992gallina} and stored in a file.
